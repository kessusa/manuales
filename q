...
def load_inventory(pf, sid, tag):
    ...
    return {"tag": tag, "groups_raw": groups,
            ...
            "sections": _secciones(rules)}


def _secciones(rules):
    """Secciones existentes del tag con nº de reglas: para colocar reglas
    nuevas donde toca y no desordenar el FW."""
    conteo = {}
    for r in rules or []:
        s = (r.get("section") or "").strip()
        if s:
            conteo[s] = conteo.get(s, 0) + 1
    return sorted(conteo.items(), key=lambda x: -x[1])[:15]
...
def candidate_rules(inv, flujo):
    ...
    for rule in inv["rules"]:
        ...
        entrada = {
            ...
            "rule_version": rule.get("rule-version"),
...
Anadir SRC/DST/SRV a una regla existente. El identificador de regla puede
ser la position (src-59) o, PREFERIBLE si se conoce, el id con prefijo id-
(src-id-1028). Ejemplo real: add to EMEA_FR_PRD_DMZA_CIS-ADMIN:srv-id-1028 TCP_455
audit: <position o id>
add to <FW name>:src-id-<id> <GRUPO>
add to <FW name>:dst-id-<id> <GRUPO>
add to <FW name>:srv-id-<id> <tcp/udp port>
add to <FW name>:comment-id-<id> <Tufin ticket>

Sustituir origen/destino en una regla:
set to <FW name>:src/dst-<rule number> <GRUPO sustituto>

Crear regla nueva (y seccion). <N> es el numero que asigne el implementador,
dejarlo como <N>, no inventar posiciones:
audit: <N>
insert to <FW name>:src-<N> <GRUPO>
insert to <FW name>:dst-<N> <GRUPO>
insert to <FW name>:srv-<N> <tcp/udp port>
insert to <FW name>:log-<N> active
insert to <FW name>:comment-<N> <Tufin ticket>
...
SECCION: para nueva_regla elige la seccion de "secciones_existentes" mas
afin por naming al flujo/aplicacion y usala en insert to ...:section-<N>.
Si ninguna encaja, propon nombre de seccion coherente con la naming e
indicalo en riesgos. No dejes <section name> sin rellenar.
...
y <N> para posiciones de reglas nuevas.
...
def build_proposal(llm_json_fn, tag, flujo, membresias, grupos_cand,
    ...
    evidencia = {
        ...
        "secciones_existentes": secciones or [],
    }
...
def analizar_candidatas(pf, sid, tag, flujo, node_info_por_ip, llm_json_fn,
                        reglas_mismo_ar=None, rol_fw=None):
    ...
    return {"tag": tag, "gestionado": True,
            ...
            "reglas_mismo_ar": hermanas,
            "secciones_existentes": inv.get("sections"),
            "propuesta": propuesta}
