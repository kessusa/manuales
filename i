...
class FortiManagerAPI:
    ...
    def preview_policy_package(self, sid, adom, package, timeout=120):
        ...
        try:
            ...
            if task_id:
                install_task = self.wait_for_task(sid, task_id, timeout)

                install_task_detail = install_task.get('line', [{}])[0].get('detail', None) if install_task.get('line', None) else None
                ...
                for target in devices:
                    ...
                    if device:
                        ...
                        # different FortiManager versions expect different
                        # attributes, so we try them in order
                        attempts = [
                            {
                                'adom': adom,
                                'flags': ['none'],
                        ...
                        for attempt in attempts:
                            ...
                            task = self.wait_for_task(sid, preview_task_id, timeout) if preview_task_id else {}

                            task_detail = task.get('line', [{}])[0].get('detail', None) if task.get('line', None) else None

                            # step 3 : retrieve the preview result using the
                            # same attributes as the attempt
                            result_data = {
                                'adom': adom
                            }

                            if attempt.get('scope', None):
                                result_data['scope'] = attempt['scope']
                            else:
                            ...
                            if message and message != 'no devices':
                                break
                        ...
                        result_dict = {
                            ...
                            'vdom': vdom,
                            'preview': message,
                            'install_task_detail': install_task_detail,
