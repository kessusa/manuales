...
UMBRAL_ESPECIFICIDAD = 50.0   # si el flujo usa <50% del alcance de la regla,
                              # la regla es demasiado amplia para este flujo
                              # -> mejor grupo/regla propia


def _num_ips(cidrs):
    """Nº de direcciones cubiertas por una lista de CIDRs/IPs."""
    total = 0
    for c in cidrs or []:
        try:
            total += ipaddress.ip_network(str(c), strict=False).num_addresses
        except ValueError:
            total += 1
    return total


def especificidad_regla(regla, flujo):
    """Determinista: % del alcance de la regla que realmente usa el flujo.
    ratio_src = IPs del flujo / IPs del grupo src de la regla (idem dst).
    Devuelve el menor de los dos (el lado mas sobredimensionado manda).
    <50% => la regla es demasiado amplia para este flujo."""
    regla_src = _num_ips((regla.get("src") or {}).get("contents"))
    regla_dst = _num_ips((regla.get("dst") or {}).get("contents"))
    flujo_src = _num_ips(flujo.get("sources"))
    flujo_dst = _num_ips(flujo.get("destinations"))

    ratio_src = (100.0 * flujo_src / regla_src) if regla_src else 100.0
    ratio_dst = (100.0 * flujo_dst / regla_dst) if regla_dst else 100.0
    ratio = min(ratio_src, ratio_dst)
    es_wide = (regla.get("afinidad") or {}).get("es_wide_rule", False)
    adecuada = ratio >= UMBRAL_ESPECIFICIDAD and not es_wide

    return {
        "ratio_pct": round(ratio, 2),
        "ratio_src_pct": round(ratio_src, 2),
        "ratio_dst_pct": round(ratio_dst, 2),
        "regla_src_ips": regla_src, "regla_dst_ips": regla_dst,
        "flujo_src_ips": flujo_src, "flujo_dst_ips": flujo_dst,
        "es_wide_rule": es_wide,
        "adecuada": adecuada,
        "recomendacion": "usar_existente" if adecuada else "crear_especifica",
    }
