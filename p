...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # ------------------------------------------------------------------
        rule_sets = obj_illumio.get_rule_sets()
        if not rule_sets:
            self.stdout.write(self.style.WARNING(
                'get_rule_sets() devolvio None/vacio para este env: la parte inline no se '
                'procesara. Revisa --region/--infra y que el endpoint de rule_sets responda 200 '
                '(en envs grandes el max_results=10000000 de get_rule_sets puede fallar).'
            ))
            rule_sets = []
