...
def build_proposal(llm_json_fn, tag, flujo, membresias, grupos_cand,
                   ...
                   rol_fw=None, sugerencia_seccion=None):
    evidencia = {
...
def analizar_candidatas(pf, sid, tag, flujo, node_info_por_ip, llm_json_fn,
    ...
    # SECCION: solo si va a haber regla NUEVA (sin cobertura total, sin
    # hermanas y sin parciales que ampliar); en el resto, ni se calcula
    # ni se manda al LLM
    sugerencia_seccion = None
    if not reglas_totales and not hermanas and not reglas_cand:
