...
class FortiManagerAPI:
    ...
    def preview_policy_package(self, sid, adom, package, timeout=120):
        ...
        try:
            ...
            # run the install preview per device
            for target in devices:
                device = target.get('name', None)
                device_oid = target.get('oid', None)
                vdom = target.get('vdom', None)

                # some FortiManager versions expect the device oid instead of its name
                device_ref = device_oid if device_oid else device
